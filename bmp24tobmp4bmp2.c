...
static uint32_t DEF_PAL_DATA[256] = {
    ...
    0xFFFF00, 0x0070FF, 0x00FFFF, 0xFFFFFF,
    0x800000, 0x808000, 0x800080, 0x008000,
    0x010101, 0x008080, 0x808080, 0x404040,
};
...
static void bmp_setpixel_2(BMP *pb, int x, int y, int c)
{
    if (x >= pb->width || y >= pb->height) return;
    int offset_byte = y * pb->stride + x * pb->cdepth / 8;
    int offset_bit  = 6 - x * pb->cdepth % 8;
    pb->pdata[offset_byte] &= ~(0x3 << offset_bit);
    pb->pdata[offset_byte] |=  (c   << offset_bit);
}
...
static void bmp24tobmp2(BMP *bmp2, BMP *bmp24)
{
    int  i, j;
    for (i = 0; i < bmp24->height; i++) {
        for (j = 0; j < bmp24->width; j++) {
            int r, g, b;
            bmp_getpixel_24(bmp24, j, i, &r, &g, &b);
            bmp_setpixel_2 (bmp2 , j, i, find_color(DEF_PAL_DATA, 4, r, g, b));
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    bmp_load(&bmp24, file);
    bmp_create(&bmpnew, bmp24.width, bmp24.height, bits);
    if (bits == 4) {
        bmp24tobmp4(&bmpnew, &bmp24);
    } else {
        bmp24tobmp2(&bmpnew, &bmp24);
    }
    DEF_PAL_DATA[0] = 0xFF00FF;
    bmp_save(&bmpnew, file);
    bmp_destroy(&bmpnew);
    bmp_destroy(&bmp24 );
    return 0;
}
